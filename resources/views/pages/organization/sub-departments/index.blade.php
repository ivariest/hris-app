...
@section('content')
    <x-common.page-shell title="Sub Departments" description="Manage sub departments under each department.">
        <x-slot:actions>
            <a href="{{ route('sub-departments.create') }}" class="inline-flex items-center justify-center rounded-xl bg-brand-500 px-4 py-3 text-sm font-medium text-white transition hover:bg-brand-600">New sub department</a>
        </x-slot:actions>
        ...
        <div class="rounded-3xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900/60 sm:p-6">
            <form method="GET" action="{{ route('sub-departments.index') }}" class="grid gap-4 lg:grid-cols-12" id="sub-department-filter-form">
                ...
                <div class="flex items-end gap-3 lg:col-span-1">
                    <button type="submit" class="inline-flex h-11 flex-1 items-center justify-center rounded-xl bg-brand-500 px-4 py-3 text-sm font-medium text-white transition hover:bg-brand-600">Go</button>
                </div>
            </form>
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    <thead class="bg-gray-50 dark:bg-gray-900/80">
                        <tr>
                            ...
                            <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Hierarchy</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                        @forelse ($subDepartments as $subDepartment)
                            <tr class="hover:bg-gray-50/80 dark:hover:bg-white/[0.02]">
                                <td class="px-5 py-4 text-sm text-gray-700 dark:text-gray-300">{{ $subDepartment->sub_department_name }}</td>
                                <td class="px-5 py-4 text-sm text-gray-700 dark:text-gray-300">
                                    {{ $subDepartment->department?->company?->company_name }} &gt; {{ $subDepartment->department?->department_name }}
                                </td>
                                <td class="px-5 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('sub-departments.show', $subDepartment) }}" class="rounded-lg border border-gray-200 px-3 py-2 text-sm font-medium text-gray-700 transition hover:border-gray-300 hover:bg-gray-50 dark:border-gray-800 dark:text-gray-300 dark:hover:border-gray-700 dark:hover:bg-white/[0.03]">View</a>
                                        <a href="{{ route('sub-departments.edit', $subDepartment) }}" class="rounded-lg border border-gray-200 px-3 py-2 text-sm font-medium text-gray-700 transition hover:border-gray-300 hover:bg-gray-50 dark:border-gray-800 dark:text-gray-300 dark:hover:border-gray-700 dark:hover:bg-white/[0.03]">Edit</a>
                                        <form method="POST" action="{{ route('sub-departments.destroy', $subDepartment) }}" onsubmit="return confirm('Delete this sub department?');">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr><td colspan="3" class="px-5 py-16 text-center text-sm text-gray-500 dark:text-gray-400">No sub departments found.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            <div class="border-t border-gray-200 px-5 py-4 dark:border-gray-800">{{ $subDepartments->links() }}</div>
        </div>
    </x-common.page-shell>
