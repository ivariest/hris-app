...
@section('content')
    <x-common.page-shell title="Employees" description="Manage employee master data and initial assignments.">
        <x-slot:actions>
            <a href="{{ route('employees.create') }}" class="inline-flex items-center justify-center rounded-xl bg-brand-500 px-4 py-3 text-sm font-medium text-white transition hover:bg-brand-600">
                New employee
            </a>
        </x-slot:actions>
        ...
        <div class="rounded-3xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900/60 sm:p-6">
            <form method="GET" action="{{ route('employees.index') }}" class="grid gap-4 lg:grid-cols-12">
                <div class="lg:col-span-5">
                    <label for="search" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Search</label>
                    <input type="text" id="search" name="search" value="{{ $search }}" placeholder="Search employee ID, name, email, or phone" class="h-11 w-full rounded-xl border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800" />
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    <thead class="bg-gray-50 dark:bg-gray-900/80">
                        <tr>
                            <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Foto</th>
                            <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Nama Karyawan | NIK</th>
                            ...
                            <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Positions</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                            ...
                            <tr class="hover:bg-gray-50/80 dark:hover:bg-white/[0.02]">
                                <td class="px-5 py-4">
                                    @if ($employee->foto_karyawan)
                                        <img src="{{ \Illuminate\Support\Facades\Storage::url($employee->foto_karyawan) }}" alt="{{ $employee->nama_karyawan }}" class="h-11 w-11 rounded-full object-cover ring-2 ring-white dark:ring-gray-900">
                                    @else
                                        <div class="flex h-11 w-11 items-center justify-center rounded-full bg-brand-50 text-sm font-semibold text-brand-600 ring-2 ring-white dark:bg-brand-500/10 dark:text-brand-300 dark:ring-gray-900">
                                ...
                                <td class="px-5 py-4 text-sm text-gray-700 dark:text-gray-300">{{ $employee->employeePosition?->position?->subDepartment?->department?->department_name ?? '-' }}</td>
                                <td class="px-5 py-4 text-sm text-gray-700 dark:text-gray-300">
                                    <div class="font-medium text-gray-900 dark:text-white/90">{{ $employee->employeePosition?->position?->position_name ?? '-' }}</div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400">{{ $employee->employeePosition?->level?->level_name ?? '-' }}</div>
                                </td>
                                <td class="px-5 py-4 text-sm text-gray-700 dark:text-gray-300">
                                    ...
                                    @if ($employee->deactivation_reason)
                                        <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ str_replace('_', ' ', $employee->deactivation_reason) }}</div>
                                    @endif
                                </td>
                                <td class="px-5 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        @if ($employee->status_karyawan === 'active')
                                            <a href="{{ route('employees.edit', $employee) }}" class="rounded-lg border border-gray-200 px-3 py-2 text-sm font-medium text-gray-700 transition hover:border-gray-300 hover:bg-gray-50 dark:border-gray-800 dark:text-gray-300 dark:hover:border-gray-700 dark:hover:bg-white/[0.03]">Edit</a>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-5 py-16 text-center text-sm text-gray-500 dark:text-gray-400">
                                    No employees found.
                                </td>
                            </tr>
                        @endforelse
